...
hht_f, spec = emd.spectra.hilberthuang(IF, IA, freq_range, mode='amplitude', sum_time=False)

signal_duration_in_seconds = int(librosa.get_duration(y=signal, sr=SIGNAL_SAMPLING))
time_vect = np.linspace(0, signal_duration_in_seconds, signal_duration_in_seconds * SIGNAL_SAMPLING)

plt.figure(figsize=(8, 10))
plt.pcolormesh(time_vect[:len(time_vect)], hht_f, spec[:, :len(time_vect)], cmap='hot_r')
